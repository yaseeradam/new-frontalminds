...
export type AdminTabType = 'dashboard' | 'projects' | 'team' | 'services' | 'testimonials' | 'settings';
...
export default function AdminSidebar({ activeTab, onTabChange }: AdminSidebarProps) {
    return (
        <aside className="lg:col-span-3">
            <nav className="bg-[#111920]/60 border border-white/10 rounded-2xl p-2 sticky top-24">
                {adminTabs.map((tab) => (
                    <button
                        key={tab.id}
                        onClick={() => onTabChange(tab.id)}
                        className={`w-full flex items-center gap-3 px-3 md:px-4 py-2.5 md:py-3 rounded-xl text-sm font-medium transition-all mb-1 ${activeTab === tab.id
